...
+-----------------------------------------------------------------------------------+
                                         ...
                                         ▼
+-----------------------------------------------------------------------------------+
|  УРОВЕНЬ 3. ЕДИНОЕ КАССОВОЕ ЯДРО & CLICK & COLLECT (Онлайн 54-ФЗ и шлюз с ERP)        |
...
### 1. Маркировка «Честный ЗНАК» и 54-ФЗ (Физическая касса и шлюз с ERP)
Полноценная фискализация (чек полного расчета) в MVP вынесена за рамки веб-приложения для снижения бюджета.
* **Как работает:** Вывод товара из оборота и печать чека происходят **на физической кассе TI** при оплате после примерки.
* **Техническая реализация:** 
  1. **Идентификация:** Клиент называет код из СМС администратору на стойке (или в отдельной зоне выдачи).
  2. **Сканирование марок:** Производится напрямую в существующей ERP системе Trend Island. Наш веб-интерфейс связывается с ERP через API для синхронизации статусов.
  3. **Пробитие чека:** Оплата при получении производится на физической кассе TI.
  4. **Онлайн-чеки:** Интеграция 54-ФЗ (CloudKassir) на стороне веб-витрины работает только для сценария предоплаты (чек «Предоплата»).
...
| **ИТОГО** | **Вилка Фазы 2 (3.5 – 4.5 мес)** | **933 – 1 500 ч** | **2 800 000 – 4 500 000 ₽** |
